...
class Post extends Component {
  ...
  componentDidUpdate(prevProps) {
    var pregFolder = this.postForScroll.current.querySelector(
      `[data-id="${this.props.selectedFolderId}"]`
    );
    var newFolder = this.postForScroll.current.querySelector(
      `[data-id="${this.props.currentAddThing}"]`
    );
    ...
    if (prevProps.currentAddThing !== this.props.currentAddThing) {
      if (
        !pregFolder.parentElement.previousElementSibling.classList.contains(
          "open"
        )
      ) {
      ...
      if (
        newFolder &&
        newFolder.parentElement.previousElementSibling.classList.contains(
          "folder"
        )
      ) {
      ...
      if (
        pregFolder.nextElementSibling &&
        pregFolder.nextElementSibling.className !== "list none"
      ) {
        if (pregFolder.nextElementSibling.lastElementChild) {
          var filePlacement = pregFolder.nextElementSibling.lastElementChild.getBoundingClientRect()
            .y;

          if (filePlacement < 120) {
            this.postForScroll.current.scrollTo({
              top:
                this.postForScroll.current.scrollTop -
                Math.abs(filePlacement) -
                220,
              behavior: "smooth"
            });
          }
    ...
    if (ev.target.className === "header") {
      if (ev.target.nextElementSibling) {
        if (ev.target.nextElementSibling.classList.contains("none")) {
          ...
          ev.target.parentElement.previousElementSibling.classList.toggle(
            "open"
          );
          this.setState({
            openedFolder: this.state.openedFolder.concat([
          ...
          if (this.props.selectedFolderId === ev.currentTarget.dataset.id) {
            ...
            opened.map((current, idx, arr) => {
              if (current === ev.currentTarget.dataset.id)
                opened.splice(idx, 1);
            });
            this.setState({
              openedFolder: opened
  ...
  folderEditBtn(ev, category) {
    ev.stopPropagation();
    ev.target.nextElementSibling.classList.toggle("none");
    if (category === "folder" || ev.currentTarget.dataset.id)
      this.props.selectedFolder(ev.currentTarget.dataset.id);
  }
  ...
  editName(ev) {
    ev.target.parentElement.parentElement.parentElement.previousElementSibling.classList.remove(
      "none"
    );
  ...
  pushEnterFunction(ev) {
    if (ev.keyCode === 13) {
      if (ev.target.value !== "") {
        this.receiveEditedName(
          ev.target.value,
          ev.target.nextElementSibling.dataset.id,
          ev
        );
      } else {
        this.receiveEditedName(
  ...
  delete(ev) {
    ...
    if (
      ev.target.parentElement.parentElement.parentElement.parentElement
        .parentElement.parentElement.previousElementSibling.dataset.id !==
        "top" &&
      ev.target.parentElement.parentElement.parentElement.parentElement
        .parentElement.parentElement.children.length === 1
    ) {
      ev.target.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.previousElementSibling.classList.remove(
        "open"
      );
    }
  }
      ...
      <div className="Post" ref={this.postForScroll}>
        <List>
          <List.Item className="forExpandWidth">
            ...
            <List.Content>
              <input
                className="editInput none"
                defaultValue={this.props.defaultFolderName}
              ...
              >
                ...
                {this.props.selectedFolderId === this.state.defaultSelected ? (
                  <Icon
                    color="red"
                    name="check circle"
                    size="small"
                  ...
                  <Icon name="check circle" size="small" data-id="top" />
                )}
                <Fragment>
                  <Button icon color="white" className="none" data-id="top">
                    <Icon
                      ...
                      onClick={this.folderEditBtn.bind(this)}
                      data-id="top"
                    />
